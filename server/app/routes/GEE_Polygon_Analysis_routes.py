...
gee_bp = Blueprint("gee", _name_)

# ✅ Earth Engine Initialization (Render compatible)
    ...
    if not ee_initialized:
        try:
            ...
            if not service_account or not private_key_json:
                raise Exception("GEE credentials missing in environment variables")
...
def wkt_to_coords(wkt_str):
    ...
    coords = [[float(lon), float(lat)] for lon, lat in
              (pair.strip().split() for pair in coords_str.split(","))]
...
def get_analysis(plan_id):
    try:
        analysis = PolygonAnalysis.query.filter_by(development_plan_id=plan_id).first()
        if not analysis:
